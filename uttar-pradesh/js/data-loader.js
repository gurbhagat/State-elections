...
function flattenForTable(constituencies, year = 2022) {
  ...
  return constituencies.map(c => {
    const acData = c[r] || {};
    const name = lang === 'hi' ? (c.nameHi || c.name) : c.name;
    const winner = lang === 'hi' ? (acData.winnerHi || acData.winner) : acData.winner;
    const runnerup = lang === 'hi' ? (acData.runnerupHi || acData.runnerup) : acData.runnerup;
    return {
      id: c.id,
      acNo: c.acNo,
      name: name,
